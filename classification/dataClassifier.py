...
def enhancedFeatureExtractorDigit(datum):
    ...
    def check_loop(x,y):
        ...
        if (x,y) in visited.keys():
            return True
        visited[(x,y)]=True
        return check_loop(x-1,y) and check_loop(x+1,y) and check_loop(x,y-1) and check_loop(x,y+1)

    for x in range(5,DIGIT_DATUM_WIDTH):
        for y in range(5,DIGIT_DATUM_HEIGHT):
            visited={}
            if datum.getPixel(x,y)==0:
                if check_loop(x,y)==True:
                    if_loop=True
                    break

    features =  basicFeatureExtractorDigit(datum)
    features["if_loop"] =if_loop
...
def enhancedPacmanFeatures(state, action):
    ...
    distance,cloest_food = min ([(util.manhattanDistance(food,pacman_position),food) for food in foods])
    features['closest_food'] = distance
    minD = 10000000
    maxD = 0
    for ghost in state.getGhostPositions():
        d=util.manhattanDistance(pacman_position,ghost) 
        minD=min(d,minD)
        maxD=max(d,maxD)
    features['close ghost'] = minD
    features['far ghost'] = maxD
    return features
