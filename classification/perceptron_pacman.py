    ...
    def train( self, trainingData, trainingLabels, validationData, validationLabels ):
        ...
        for iteration in range(self.max_iterations):
            ...
            for i in range(len(trainingData)):
                ...
                data = trainingData[i]
                score = util.Counter()
                state, legalMoves = data[0],data[1]
                for l in legalMoves:
                    score[l] = self.weights*state
                best_label = score.argMax()
                training_label = trainingLabels[i]
                if training_label != best_label:
                    trainingDatum = trainingData[i][0]
                    self.weights -= trainingDatum[best_label]
                    self.weights += trainingDatum[training_label]
